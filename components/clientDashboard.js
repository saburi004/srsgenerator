...
Primary: ${colorThemes.find(t => t.name === selectedTheme)?.text}
  ...
  const saveEdit = async () => {
   ...
   useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      generateImages(aiPrompt);
    }, 1000); // waits 1 sec after typing stops
  ...
  const generateImages = async (text) => {
    try {
      const res = await fetch("/api/deepai-image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
{/* images section */}
  <div className="max-w-md w-full bg-gray-900 p-6 rounded-xl shadow-lg border border-gray-700">
        <h1 className="text-xl text-white font-bold mb-2">Generated UI Screens</h1>
